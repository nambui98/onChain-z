...
export default function RightContent({ clips, setClipActive }: { clips: Clip[], setClipActive: Function }) {
  const list = clips

  return (
    <div className="RightContent Widget w-2/5 flex flex-col gap-y-6 sm-max:w-full sm-max:mt-4">
      {list.map((r, idx) => (
        <RightWidgetListItem setClipActive={setClipActive} clip={r} />
      ))}
    </div>
  )
}

const RightWidgetListItem = ({ clip, setClipActive }: { clip: Clip, setClipActive: Function }) => {
  return (
    <div className="group flex gap-x-4 items-center cursor-pointer" onClick={() => { setClipActive(clip) }}>
      {/* <a href={clip.url}> */}
      <div
