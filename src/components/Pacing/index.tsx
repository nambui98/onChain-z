...
export default function Pacing({ className, total, pageSize, currentPage, originURL }: Props) {
            ...
            {/* // TODO: Thach: panigation*/}
            {
                listPage.map((page, index) => {
                    if (typeof page !== 'string' && page === currentPage) {
                        return <a href={originURL + '?page=' + page}><ButtonPrimary className="w-40px h-40px rounded-lg font-normal" onClick={() => { }}>
                            {page}
                        </ButtonPrimary></a>
                    } else {
                        return <a href={originURL + '?page=' + getURL(page, currentPage, listPage[index + 1])}><ButtonOutline className="w-40px h-40px rounded-lg font-normal" onClick={() => { }}>
                            {page}
                        </ButtonOutline>
                        </a>
                    }
