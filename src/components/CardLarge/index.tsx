...
export default function CardLarge({ article, isNotShowAuthor, isNotShowDescription, classNameImage, classNameTitle }: CardProps) {
  ...
  return (
    ...
    <div
      className="Card group transition duration-300 ease-out cursor-pointer  bg-no-repeat bg-cover relative flex justify-between items-start mb-6"
    >
      <div>
        <p className={`text-base  bottom-1.5rem  inset-x-1.5rem text-grey font-semibold transition duration-300 ease-out group-hover:text-primary ${classNameTitle ?? ''}`}>{article?.title}</p>
        {
