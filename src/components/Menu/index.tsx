      ...
      <ul className="PrimaryNav with-indicator uppercase font-600 z-2">
            ...
            // * * Success*/}
        {
          menu_data.map((menu, index) => {
            ...
            }} key={index} info={menu} active={menu.path === currentPath} />
          })
        }
      </ul>
      ...
      <div className="w-386px sm-max:w-228px pl-15">
        <form className="w-full">
          <label class="relative block">
            <input ref={refInput} placeholder="Tìm kiếm..." type="text" className={`animationPlace absolute right-0 opa transform -translate-y-1/2 top-0 p-0 py-10px bg-transparent border-0 border-b-1 border-primary text-sm font-semibold transition duration-1000 ease-out ${activeSearch ? 'w-full' : 'w-0'} ${activeSearch ? 'opacity-100 active' : 'opacity-0'} placeholder-slate-400 focus:outline-none focus:border-0 focus:border-b-1 focus:border-primary focus:ring-0 `} style={{
