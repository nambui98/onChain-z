  ...
  return (
    <div className="rounded-md overflow-hidden shadow-sm p-4  transition-transform h-auto">
      <a href={`/post/${slug}`}>
        ...
        <div>
          <h1 className="heading1 my-2 font-bold text-xl text-gray-900">{title}</h1>
        ...
        <div className="flex justify-between my-4 items-center">
          <div className="flex space-x-2 items-center overflow-hidden">
            <img
              ...
              src={author?.bioImage?.url ? `http://localhost:1337${author?.bioImage?.url}` : 'https://via.placeholder.com/1080'}
              alt="author picture"
            />
            <p className="font-medium text-xs text-gray-600 cursor-pointer">{author?.name}</p>
          </div>
          <div class="inline-flex rounded-md ">
            <a href={`/post/${slug}`} class="inline-flex items-center justify-center px-5 py-2 border border-transparent text-base font-medium rounded-md text-white bg-yellow-500 hover:bg-yellow-400">
              Read article
            </a>
          </div>
        </div>
